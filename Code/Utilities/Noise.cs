...
namespace Earthian.Utilities
{
    ...
    public class Noise
    {
            ...
            public Noise(double pScale, byte pOctaves)
            {
                XScale = pScale;
                YScale = pScale;
                Octaves = pOctaves;
            ...
            public Noise(double pXScale, double pYScale, byte pOctaves)
            {
                XScale = pXScale;
                YScale = pYScale;
                Octaves = pOctaves;
            ...
            {
                if (Octaves > 1)
                    return NoiseGen.GetOctaveNoise(x * XScale, y * YScale, z * ZScale, Octaves);
                else
                    return NoiseGen.GetNoise(x * XScale, y * YScale, z * ZScale);
            }

            public float GetNoise(double x, double y)
            {
                if (Octaves > 1)
                    return NoiseGen.GetOctaveNoise(x * XScale, y * YScale, Octaves);
                else
                    return NoiseGen.GetNoise(x * XScale, y * YScale);
            }

            public float GetNoise(double x)
            {
                if (Octaves > 1)
                    return NoiseGen.GetOctaveNoise(x * XScale, Octaves);
                else
                    return NoiseGen.GetNoise(x * XScale);
            }
        }
        ...
        public static class NoiseGen
        {
            ...
            /// <param name="x"></param>
            /// <returns></returns>
            public static float GetNoise(double x)
            {
                int i0 = fastfloor(x);
                int i1 = i0 + 1;
                double x0 = x - i0;
            ...
            public static float GetNoise(double x, double y)
            {
                ...
                return 40.0f * ((float)(n0 + n1 + n2)); // TODO: The scale factor is preliminary!
            }
            // 3D simplex noise
            ...
            // get multiple octaves of noise at once
            public static float GetOctaveNoise(double pX, int pOctaves)
            {
                float value = 1f;
                float divisor = 0;
                float currentHalf = 0;
                ...
                for (int i = 0; i < pOctaves; i++)
                {
                    currentHalf = (float)Math.Pow(0.5f, i);
                    currentDouble = (float)Math.Pow(2, i);
                    value *= GetNoise(pX * currentDouble) * currentHalf;
                    divisor += currentHalf;
                }
            ...
            // get multiple octaves of noise at once
            public static float GetOctaveNoise(double pX, double pY, int pOctaves)
            {
                float value = 0;
                float divisor = 0;
                float currentHalf = 0;
                ...
                for (int i = 0; i < pOctaves; i++)
                {
                    currentHalf = (float)Math.Pow(0.5f, i);
                    currentDouble = (float)Math.Pow(2, i);
                    value += GetNoise(pX * currentDouble, pY * currentDouble) * currentHalf;
                    divisor += currentHalf;
                }
